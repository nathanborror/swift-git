...
extension git_checkout_strategy_t: @retroactive OptionSet {}
...
final class CheckoutOptions {
    init(
        ...
        progressCallback: ((CheckoutProgress) -> Void)? = nil
    ) {
...
private func checkoutProgressCallback(
    ...
    checkoutOptions.progressCallback?(
        CheckoutProgress(path: pathString, completedSteps: completedSteps, totalSteps: totalSteps)
    )
}
