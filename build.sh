...
function copy_modulemap() {
    local FWDIRS=$(find CGit2.xcframework -mindepth 1 -maxdepth 1 -type d)
    for d in ${FWDIRS[@]}; do
        echo $d
        cp CGit2.modulemap $d/Headers/module.modulemap
    done
}

### Build libgit2 and CGit2 frameworks for all available platforms
...
build_xcframework libgit2_all install CGit2
cd $REPO_ROOT
copy_modulemap
